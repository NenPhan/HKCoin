...
class _PopupDropdownFieldState<T> extends State<_PopupDropdownField<T>> {
            ...
            children: [
              Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    filled: true,
                    fillColor: widget.searchFieldBackgroundColor ?? Colors.grey.shade100,
                    hintText: 'Tìm kiếm...',
                  ...
                  onChanged: (value) {
                    setModalState(() {
                      filteredItems = widget.items
                          .where((item) => widget.itemLabel(item).toLowerCase().contains(value.toLowerCase()))
                          .toList();
                    });
                  },
                ),
              ...
              if (widget.showClearButton && !widget.isMultiSelect)
                ListTile(
                  ...
                  title: Text(tr(widget.cleartext ?? 'Bỏ chọn'), style: const TextStyle(color: Colors.red)),
                  onTap: () {
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final style = widget.itemTextStyle ?? TextStyle(color: widget.textColor ?? Colors.black);
                    ...
                    if (widget.isMultiSelect) {
                      ...
                      return ListTile(
                        ...
                        trailing: selected == item ? const Icon(Icons.check, color: Colors.green) : null,
                        onTap: () {
                          Navigator.pop(context);
                          setState(() => selected = item);
                          widget.field.didChange(item); // Cập nhật FormFieldState
  ...
  Widget build(BuildContext context) {
    final displayText = widget.isMultiSelect
        ? (widget.selectedItems?.map(widget.itemLabel).join(', ') ?? tr(widget.placeholder ?? 'Chọn...'))
        : selected != null
            ? widget.itemLabel(selected!)
            : tr(widget.placeholder ?? 'Chọn...');
    print(widget.height);
              ...
              children: [
                Expanded(
                  child: Text(
                    displayText,
                    style: TextStyle(
                      color: (selected != null || widget.isMultiSelect)
