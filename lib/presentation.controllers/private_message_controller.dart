...
class PrivateMessageController extends GetxController {
  ...
  Future gePrivateMessagesData({int page = 1, bool isLoadMore = false, bool isRead = false, int limit = 15}) async {
    if (isRead) {
    ...
    try{      
        ...
        await handleEither(
          await MessageRepository().getPrivateMessages(isRead: isRead, page: page, limit: limit),
          (r) {
            if (isRead) {
              if (isLoadMore) {
                readMessagesPagination?.privateMessages?.addAll(r.privateMessages ?? []);
                readMessagesPagination = r;
              } else {
                readMessagesPagination?.privateMessages?.assignAll(r.privateMessages ?? []);
                readMessagesPagination = r;
              }
            }else{
              if (isLoadMore) {
                newMessagesPagination?.privateMessages?.addAll(r.privateMessages ?? []);
                newMessagesPagination = r;
              } else {
                newMessagesPagination?.privateMessages?.assignAll(r.privateMessages ?? []);
                newMessagesPagination = r;
              }
            }                  
          },
