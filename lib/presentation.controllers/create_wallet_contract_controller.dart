...
class CreateWalletWithContractController extends GetxController {
  ...
  final TextEditingController contractSymbolController =
      TextEditingController();
  final TextEditingController contractDecimalController =
      TextEditingController();
    ...
    await handleEitherReturn(
      await WalletRepository().createCustomerToken(
        CustomerWalletToken(
          ...
          decimals:
              contractDecimalController.text.trim().isNotEmpty
                  ? int.parse(contractDecimalController.text.trim())
                  : 18,
  ...
  Future<void> updateContractInfomation() async {
    ...
    try {
      ...
      errorMessage.value =
          'Địa chỉ hợp đồng không đúng định dạng: ${e.toString()}';
    }
